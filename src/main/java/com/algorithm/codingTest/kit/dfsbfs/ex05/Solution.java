...
/**
 * dfs로 풀긴했는데 효율성 테스트에서 안됨
 * dfs는 일단 한방향으로 끝까지 들어가보고 막히면 돌아와서 다시 검색하는 과정이기때문에 효율성이 떨어짐
 * bfs로 풀어야함
 */
public class Solution {
    ...
    public void dfs(int[][] maps, int[][] visited, int x, int y, int step) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (nextX >= 0 && nextY >= 0 && nextX < maps[0].length && nextY < maps.length) {
                if (maps[nextY][nextX] == 1 && visited[nextY][nextX] != 1) {
                    ...
                    dfs(maps, visited, nextX, nextY, step + 1);
