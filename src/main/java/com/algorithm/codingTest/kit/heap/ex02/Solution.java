package com.algorithm.codingTest.kit.heap.ex02;
...
public class Solution {
	...
	public int solution(int stock, int[] dates, int[] supplies, int k) {
		int answer = 0;
		PriorityQueue<Integer> suppliesQueue = new PriorityQueue<Integer>((a, b) -> b - a);

		int cnt = 0;
		for(int i = 0; i < k; i++) {
			if(cnt < dates.length && i == dates[cnt]) {
				suppliesQueue.add(supplies[cnt]);
				cnt++;
			}
			if(stock == 0) {
				stock = suppliesQueue.remove();
				answer++;
			}
			stock--;
		}
